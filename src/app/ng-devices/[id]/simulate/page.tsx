  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const pidObj = typeof entry.processId === "object" ? entry.processId : null;
        const pidIdRaw = pidObj?._id || entry.processId || entry.process?._id || "";
        const pidId = typeof pidIdRaw === "string" ? pidIdRaw : "";

        const isObjectId = (val: any) => typeof val === "string" && /^[a-fA-F0-9]{24}$/.test(val);

        if (!pidId || !isObjectId(pidId)) {
          // Check if pidObj is already a full process object
          if (pidObj && pidObj.stages) {
            // already have it
          } else {
            setError("Process ID not available or invalid for this device");
            setLoading(false);
            return;
          }
        }
        ...
        let proc = pidObj;
        if (!proc || !proc.stages) {
          proc = await getProcessByID(String(pidId));
        }
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="flex items-center gap-2 p-4">
        ...
        {(deviceInfo?.productId?.name || deviceInfo?.deviceInfo?.modelName || deviceInfo?.deviceId?.modelName || deviceInfo?.device?.model) && (
          <span className="ml-auto text-xs text-gray-500">
            Model: {deviceInfo?.productId?.name || deviceInfo?.deviceInfo?.modelName || deviceInfo?.deviceId?.modelName || deviceInfo?.device?.model}
          </span>
        )}
      </div>
