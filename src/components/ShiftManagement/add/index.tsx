...
const AddShiftManagement = () => {
  ...
  const calculateTimeDifference = (start: string, end: string) => {
    ...
    if (diff < 0) diff += 1440; // Add 24 hours if end time is earlier than start time (e.g. 21:00 to 00:15)
    return diff;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use logical order instead of chronological sort to support night shifts (midnight crossing)
      ...
      const errorMessage = err?.message || "Failed to create shift. Please try again.";
