...
export default function DeviceTestComponent({
    ...
    const fetchOC = async () => {
      ...
      if (ocNo) {
        try {
          ...
          console.error("Error fetching order confirmation in component:", error);
        }
      }
    };
    fetchOC();
  }, [processData?.orderConfirmationNo, product?.orderConfirmationNo]);
  ...
  const cartonLabelData = React.useMemo(() => {
    ...
    const parsedL = parseFloat(String(l || "0"));
    const parsedW = parseFloat(String(w || "0"));
    const parsedH = parseFloat(String(h || "0"));
    const dimensionsCm = (parsedL > 0 && parsedW > 0 && parsedH > 0)
      ? `${parsedL}*${parsedW}*${parsedH}CM`
      : "N/A";

    const customerName = resolveIdentityValue(
    ...
    const modelName = resolveIdentityValue(
      orderConfirmationData?.modelName,
      processData?.modelName,
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-6 text-emerald-900 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-5">
                <div className="relative">
                  <div className="absolute -inset-1 animate-pulse rounded-full bg-emerald-400/20 blur-sm"></div>
                  <div className="relative flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 shadow-inner">
                    <CheckCircle className="h-8 w-8" />
                  </div>
                </div>
                <div className="flex-1">
                  <h4 className="flex items-center gap-2 text-xl font-black uppercase tracking-tight text-emerald-800">
                    Stage Completed
                    <span className="rounded-full bg-emerald-500/10 px-3 py-0.5 text-[10px] font-extrabold text-emerald-600 ring-1 ring-emerald-500/20">
                      HISTORY DETECTED
                    </span>
                  </h4>
                  <p className="mt-1 text-base font-semibold text-emerald-600/90 leading-relaxed">
                    This unit has already been tested and passed in this stage.
                  </p>
                </div>
                <div className="hidden sm:block">
                  <div className="rounded-xl bg-white/50 px-4 py-2 ring-1 ring-emerald-100">
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 block">Current Status</span>
                    <span className="text-sm font-black text-emerald-700">COMPLETED</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        processData={{
          ...
          customerName:
            [
              ...
              product?.selectedProduct?.name,
              product?.name,
            ].find((c) => !isMissingIdentityMarker(c)) || "Production Order",
          modelName:
            [
              orderConfirmationData?.modelName,
              planingAndScheduling?.modelName,
              ...
              product?.selectedProduct?.name,
              product?.name,
            ].find((m) => !isMissingIdentityMarker(m)) || "N/A",
        }}
        product={product}
