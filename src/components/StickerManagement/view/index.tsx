...
const ViewStickerManagementComponent = () => {
  ...
  const [stickerFieldId, setStickerFieldId] = React.useState("");
  ...
  const getStickerField = async () => {
    try {
      ...
      setStickerFieldData(result.data);
    } catch (error) {
  ...
  const handleSubmitSticker = async () => {
    try {
      ...
      if (stickerFieldId != "") {
        formData.append("stickerFieldId", stickerFieldId);
      }
      const result = await createStickerField(formData);
      if (result && result.status == 200) {
        setIsModalOpen(false);
        getStickerField();
        toast.success(result.message || "Sticker Field Created Successfully");
      }
      return false;
    } catch (error) {
      console.log("Error Creating Submit Sticker ==>", error);
    }
  };
  const handleAddStickerManagement = () => {
    setFieldName("");
    setStickerFieldId("");
    setIsModalOpen(true);
  };
        ...
        {loading ? (
          ...
          <>
            <div className="mb-4 mt-4 flex justify-end gap-3 text-right text-end">
              <button
                onClick={() => handleAddStickerManagement()}
            ...
            <Modal
              isOpen={isModalOpen}
              onSubmit={handleSubmitSticker}
              onClose={closeModal}
              title={
                stickerFieldId ? "Update Sticker Field" : "Add Sticker Field"
              }
            >
